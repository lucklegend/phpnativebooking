...
if(isset($_POST['szID']))
{
	$query  = "SELECT * FROM user_account  WHERE username = '". mysqli_real_escape_string($_POST['szID'])."' AND password = '". mysqli_real_escape_string($_POST['szPassword'])."' AND active = '1'";
	$result = mysqli_query($conn,$query) or die(mysqli_error($conn)) ;
...
Ardmore Park redefines modern luxury living with a stylish lavishness and exclusive address that is the definitive residence for the well established. The sprawling 8 acres make Ardmore Park one of the largest freehold sites in a prime residential location in Singapore and it is set on high land enjoying island wide views.</p>
  ...
  if (isset($_SESSION['basic_is_logged_in']) && isset($_GET['crypted']))
  {
  ?>?crypted=<?php echo $_GET['crypted']; } ?>"><img src="img/rgtvirtual.jpg" width="163" height="132" border="0"></a></td>
        </tr>
        <tr>
          <td><a href="gallery.php<?php
  if (isset($_SESSION['basic_is_logged_in']) && isset($_GET['crypted']))
  {
  ?>?crypted=<?php echo $_GET['crypted']; } ?>"><img src="img/rgtphotol.jpg" width="163" height="141" border="0"></a></td>
        </tr>
        <tr>
          <td><a href="facsiteplan.php<?php
  if (isset($_SESSION['basic_is_logged_in']) && isset($_GET['crypted']))
  {
  ?>?crypted=<?php echo $_GET['crypted']; } ?>"><img src="img/rgtplan.jpg" width="163" height="137" border="0"></a></td>
        </tr>
    </table>
