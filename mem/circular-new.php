...
//				if($width > $max_width || $height > $max_height)
	...
	$query = "INSERT INTO circular(date_start, section, title, contents, attachment) VALUES ('$date', '$section', '$title', '$text', '$new_name')";
	$result = mysql_query($query) or die(mysql_error()) ; 
...
height="3"></SPACER></td>
		...
		<form name="form1" onsubmit="return validate()" method="post" encType="multipart/form-data" action="circular-new.php?crypted=<? echo $_GET['crypted']; ?>">
			<table cellSpacing="0" cellPadding="0" align="left" border="0" id="table8">
				<tr>
					<td class="bk" width="100">Date</td>
					<td class="bk2"><script>DateInput('bulletin_date', true, 'YYYY-MM-DD')</script></td>
				</tr>
				<tr>
					<td class="bk">Section</td>
					<td class="bk2">
					<select id="section" onchange="enable()" name="section">
					<option value="Community News" selected>Community News
					</option>
				...
				<tr vAlign="top">
					...
					<br>
					<textarea class="txt1" onkeyup="count()" name="text" rows="14" cols="60"></textarea>
					<br>
