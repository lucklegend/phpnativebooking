...
if(isset($_POST[submit]))
{
	...
	$query  = "INSERT INTO table_barred (user_id, facility_id, last_booking, bar_expiry) VALUES ('$user_id', '$facility_id', '$last_booking', '$bar_expiry')";
	$result = mysql_query($query) or die(mysql_error()) ;
	echo "<script type=text/javascript language=javascript> window.location.href = 'barring.php?crypted=$_GET[crypted]'; </script>"; 
}
...
height="3"></SPACER></td>
  ...
  <? } ?>
        ...
        <tr>
		  ...
		  <? 
		  $sqlusers = "SELECT * FROM user_account ORDER BY name ASC";
		  $resultusers = mysql_query($sqlusers);
		  while ($rowusers = mysql_fetch_array($resultusers))
		  {
		  ?>
          <option value="<? echo $rowusers['id']; ?>"><? echo $rowusers['name']; ?></option>
		  <?
		  }
		  ?>
          </select></td>
