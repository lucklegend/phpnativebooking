...
if(isset($_POST['update']))
{
	...
	{
	$query = "UPDATE useful_info SET telephone = '$telephone', emergency = '$emergency', fax = '$fax', display_name = '$display_name', address = '$address', url = '$url' WHERE id = '$explodedids[0]'";
	$result = mysql_query($query) or die(mysql_error()) ; 
	}
	else
	{
		...
		while ($i < $j)
		{
	$query = "UPDATE useful_info SET telephone = '$telephone', emergency = '$emergency', fax = '$fax', display_name = '$display_name', address = '$address', url = '$url' WHERE id = '$explodedids[$i]'";
	$result = mysql_query($query) or die(mysql_error()) ; 
		$i++;
		}
	}
	...
	if (substr_count($category, ",") > 1)
	{
		...
		$categorysql = "SELECT * FROM useful_info WHERE category = '$morecategory[$i]' AND display_name = '$display_name'";
		$categoryresult = mysql_query($categorysql);
		$catenum = mysql_num_rows($categoryresult);
		
		if ($catenum == 0)
		{
		$query = "INSERT INTO useful_info(category, telephone, emergency, fax, display_name, address, url) VALUES ('$morecategory[$i]', '$telephone', '$emergency', '$fax', '$display_name', '$address', '$url')";
	$result = mysql_query($query) or die(mysql_error()) ; 
		}
		$i++;
	...
	echo "<script type=text/javascript language=javascript> window.location.href = 'useful.php?crypted=" . $_GET['crypted'] . "'; </script> ";
		...
		<form name="form1" onSubmit="return validate()" method="post" action="useful-edit.php?crypted=<? echo $_GET['crypted']; ?>">
			...
			<table cellSpacing="0" cellPadding="0" align="left" border="0" id="table8">
				<tr>
					...
					$anotherresult = mysql_query($anothersql);
					$anotheramt = mysql_num_rows($anotherresult);
					?>
                    <select style="width: 13.6em" onclick="delSelected(this)" id="category" size="3" name="category">
                    <option value="<? echo $newrow['category']; ?>"><? echo $newrow['category']; ?></option>
					<?
                    if ($anotheramt == 0)
					{
					...
					else
					{
						...
						?>
                    <option value="<? echo $anotherrow['category']; ?>"><? echo $anotherrow['category']; ?></option>
                        <?						
						}
					}
					?>
				...
				<tr>
					...
					<td class="bk2rbdr">
					<input class="txt1" id="telephone" onKeyDown="enable()" maxLength="20" name="telephone" value="<? echo $newrow['telephone']; ?>"></td>
				</tr>
				<tr>
					<td class="bk">Emergency:</td>
					<td class="bk2rbdr">
					<input class="txt1" id="emergency" onKeyDown="enable()" maxLength="20" name="emergency" value="<? echo $newrow['emergency']; ?>"></td>
				</tr>
				<tr>
					<td class="bk">Fax:</td>
					<td class="bk2rbdr">
					<input class="txt1" id="fax" onKeyDown="enable()" maxLength="20" name="fax" value="<? echo $newrow['fax']; ?>"></td>
				</tr>
				<tr>
					<td class="bk">Display Name:</td>
					<td class="bk2rbdr" colSpan="2">
					<input id="display_name" onKeyDown="enable()" style="width: 29em" maxLength="50" name="display_name" value="<? echo $newrow['display_name']; ?>"></td>
				</tr>
				<tr>
					<td class="bk" vAlign="top">Address:</td>
					<td class="bk2" colSpan="2">
					<textarea onKeyDown="enable()" style="width: 27.6em" name="address" rows="3" cols="20"><? echo $newrow['address']; ?></textarea></td>
				</tr>
				<tr>
					<td class="bk">Website:</td>
					<td class="bk2" colSpan="2">
					<input class="txt1" id="url" onKeyDown="enable()" maxLength="200" name="url" value="<? echo $newrow['url']; ?>"></td>
				</tr>
				<tr align="middle">
