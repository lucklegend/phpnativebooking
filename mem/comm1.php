...
source.style.backgroundColor=originalcolor
		...
		?>
          <table width="100%" border="0"  id=ignore>
                <tr>
...
$lastyear = $year - 1;
$nextyear = $year + 1;
echo "<table summary=\"Monthly calendar\"  onMouseover= changeto('#FBE694') onMouseout= changeback('white')  width = 100% cellspacing= 2 cellpadding= 2 id= ignore class=\"sk_bok_green\">
<caption ><a href=comm.php?crypted=$_GET[crypted]&calender&lastyear=$lastyear ><font color=black>Last Year</font></a>&nbsp;&nbsp;&nbsp;<a href=comm.php?crypted=$_GET[crypted]&calender&last=$lastmonth&year=$year><font color=black>Last Month</a>&nbsp;&nbsp;&nbsp; <b>[$month_caption ]</b> &nbsp;&nbsp;&nbsp;<a href=comm.php?crypted=$_GET[crypted]&calender&next=$nextmonth&year=$year><font color= black>Next Month</font></a> &nbsp;<a href=comm.php?crypted=$_GET[crypted]&calender&lastyear=$nextyear><font color= black>Next Year<font></a><br></caption>
<tr align=center id=ignore>
<th width =308 id=ignore bgcolor=#FFC56C><font color =Red>Sun</font></th>
<th width =308 id=ignore bgcolor=#FFC56C><font color =black>Mon</font></th>
<th width =308 id=ignore bgcolor=#FFC56C><font color =black>Tue</font></th>
<th width =308 id=ignore bgcolor=#FFC56C><font color =black>Wed</font></th>
<th width =308 id=ignore bgcolor=#FFC56C><font color =black>Thu</font></th>
<th width =308 id=ignore bgcolor=#FFC56C><font color =black>Fri</font></th> 
<th width =308 id=ignore bgcolor=#FFC56C><font color =black>Sat</font></th> 
</tr>\n"; 
...
{
print "     <td id=ignore width =308 height=100  style='border-width:1px; border-style:solid; border-color:#CCCCCC;'>&nbsp;</td>";  // Empty slots 
$slot++;
}
	if($day == '')
	{
...
print "<tr align=center width =50>\n"; // move on to next row
	...
	{	$msg .= "<table id=ignore width=100%>";
		...
		if (strtoupper($row[heading]) == 'PUBLIC HOLIDAY')
		{
			$red = 1;
		}
		
		$msg .= "<tr ><td  id=ignore><strong>$row[heading]&nbsp;</strong></td></tr><tr><td id=ignore><a href=\"comm.php?crypted=$_GET[crypted]&last=$month_no&year=$db_date_search[2]&calender&edit_event=$row[sno]\" $popup_link);return false\">$row[details]</a></td></tr>";
		}
		$msg .= "</table>";
	}else
	{
	...
	if ($red >= 1)
	{
	echo "<td  $color id=$idd width =308 height=100 valign=top  style='border-width:1px; border-style:solid; border-color:#CCCCCC;'><DIV align=center id= tips><a href = #><font color=black><a href=comm.php?crypted=$_GET[crypted]&last=$month_no&year=$db_date_search[2]&calender&add_event=$db_date_search[0]-$db_date_search[1]-$db_date_search[2]><font color=red>$day</font></a> </font>
		
		 ";
	}
	else
	{
	echo "<td  $color id=$idd width =308 height=100 valign=top  style='border-width:1px; border-style:solid; border-color:#CCCCCC;'><DIV align=center id= tips><a href = #><font color=black><a href=comm.php?crypted=$_GET[crypted]&last=$month_no&year=$db_date_search[2]&calender&add_event=$db_date_search[0]-$db_date_search[1]-$db_date_search[2]><div align='center' style='background-color:#CCCC66; color:#000000;'>$day	</div></a> </font>
