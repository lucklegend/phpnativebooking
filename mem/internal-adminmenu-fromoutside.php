        <tr> 
          <td class="leftdecline" height="3"><spacer type="block" 
height="3"><b><br>
            &nbsp;&nbsp;User</b></spacer></td>
        </tr>
        <tr> 
          <td class="leftcontent" height="69">&nbsp;&nbsp;<img height="7" src="img/leftdot.gif" width="9" border="0"> <a href="mem/admin.php?crypted=<? echo $_GET['crypted']; ?>">Main Menu </a><br>
            &nbsp;&nbsp;<img height="7" src="img/leftdot.gif" width="9" border="0"> <a class="copy" href="mem/admin.php?mode=password&crypted=<? echo $_GET['crypted']; ?>">Change 
            Password </a><br>
          </td>
        </tr>
        <tr> 
          <td class="leftdecline" height="3"><spacer type="block" 
height="3"><b><br>
            &nbsp;&nbsp;System Management</b></spacer></td>
        </tr>
        <tr> 
          <td class="leftcontent" height="3"><br>
            &nbsp;&nbsp; <img height="7" src="img/leftdot.gif" width="9" border="0"> <a href="mem/facility.php?crypted=<?php echo $_GET[crypted]; ?>&page=user">Manage Users</a><br>
            &nbsp;&nbsp; <img height="7" src="img/leftdot.gif" width="9" border="0"> <a href="mem/booking.php?crypted=<?php echo $_GET[crypted]; ?>&cr=1">Create Facilities</a><br>
            &nbsp;&nbsp; <img height="7" src="img/leftdot.gif" width="9" border="0"> <a href="mem/booking.php?crypted=<?php echo $_GET[crypted]; ?>&page=view">Manage Facilities</a><br>
            &nbsp;&nbsp; <img height="7" src="img/leftdot.gif" width="9" border="0"> <a href="mem/barring.php?crypted=<?php echo $_GET[crypted]; ?>">Facility Barring</a><br>
            &nbsp;&nbsp; <img height="7" src="img/leftdot.gif" width="9" border="0"> <a href="mem/comm.php?crypted=<? echo $_GET['crypted']; ?>&calender"> 
            Calendar / Events</a><br>
            <br>
          </td>
        </tr>
        <tr> 
          <td class="leftdecline" height="3"><spacer type="block" 
height="3"><b><br>
            &nbsp;&nbsp;Content Management</b></spacer></td>
        </tr>
        <tr> 
          <td class="leftcontent" height="3"><br>
            &nbsp;&nbsp; <img height="7" src="img/leftdot.gif" width="9" border="0"> 
            <a href="armenities.php?crypted=<? echo $_GET['crypted']; ?>">Amenities</a><br>
            &nbsp;&nbsp; <img height="7" src="img/leftdot.gif" width="9" border="0"> <a href="mem/addforms.php?crypted=<? echo $_GET['crypted']; ?>">Application 
            Forms</a><br>
            &nbsp;&nbsp; <img height="7" src="img/leftdot.gif" width="9" border="0"> <a href="mem/circular.php?crypted=<? echo $_GET['crypted']; ?>">News / Circulars</a><br>
            &nbsp;&nbsp; <img height="7" src="img/leftdot.gif" width="9" border="0"> <a href="mem/useful.php?crypted=<? echo $_GET['crypted']; ?>">Useful Info</a><span class="txtgrey"></span><a href="http://helpdesk.axonhq.net" target="_blank"> 
            </a><a href="mem/comm.php?crypted=<? echo $_GET['crypted']; ?>&calender"> 
            </a><br>
            <br>
          </td>
        </tr>

       <tr> 
          <td class="leftdecline" height="3"><spacer type="block" 
height="3"><b><br>
            &nbsp;&nbsp;Facility Bookings</b></spacer></td>
        </tr>
        <tr> 
          <td class="leftcontent" height="3"><br>
            &nbsp;&nbsp; <img height="7" src="img/leftdot.gif" width="9" border="0">  <a href="mem/booking.php?crypted=<?php echo $_GET[crypted]; ?>&page=book_now&user_id=<? echo $_SESSION['basic_is_logged_in']; ?>&pid=6"> Online Booking</a><br>
        	</td>
        </tr>
                
        <tr> 
          <td class="leftdecline" height="3"><SPACER type="block" 
height="3"><b><br>
            &nbsp;&nbsp;Reports<br>
            <br>
            </b>&nbsp; <img height="7" src="img/leftdot.gif" width="9" border="0"> <a href="mem/booking.php?crypted=<?php echo $_GET[crypted]; ?>&page=all">Booking Report</a><br>
            <b><br>
            </b></SPACER></td>
        </tr>
        <tr> 
          <td class="leftdecline" height="21"><b>&nbsp;&nbsp;AXON Support</b></td>
        </tr>
        <tr> 
          <td class="leftdecline" height="3"><br>
            &nbsp;&nbsp; <img height="7" src="img/leftdot.gif" width="9" border="0"> 
            <a href="http://helpdesk.axonhq.net" target="_blank">Axon Helpdesk</a><br>
            <br>
          </td>
        </tr>
        <tr> 
          <td class="leftdecline" height="3"> 
            <div align="center"><font size="2"><b> Axon Booking Ver 3.0</b></font></div>
          </td>
        </tr>
        <tr> 
          <td class="leftdecline" height="3"> 
            <div align="center"><font size="2"><br>
              Developed by<br>
              Axon Consulting<br>
              www.axon.com.sg<br>
              <br>
              Tel: +65 6344 9618<br>
              Fax: +65 6344 9766<br>
              e: info@axon.com.sg</font></div>
          </td>
        </tr>